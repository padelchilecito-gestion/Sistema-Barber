      ...
      <div className={`flex-none mb-4 flex items-center justify-between ${isGuestMode ? 'border-b border-slate-800 pb-4' : ''}`}>
          ...
          {isGuestMode && (
              <button 
                ...
                className="text-xs bg-slate-800 text-slate-500 hover:text-amber-500 px-3 py-1.5 rounded-full border border-slate-700 transition-colors flex items-center gap-1"
                title="Acceso para el dueño"
              >
                  <LogIn size={14} /> <span className="hidden sm:inline">Soy Admin</span>
              </button>
          )}
      </div>
                  ...
                  <div className={`flex flex-col max-w-[90%] md:max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`px-5 py-3.5 rounded-2xl relative shadow-md text-sm md:text-base ${
                          ...
                          {msg.role === 'assistant' && !isGuestMode && (
                              <button onClick={() => copyToClipboard(msg.text)} className="absolute top-2 right-2 opacity-0 hover:opacity-100 transition-opacity bg-black/10 hover:bg-black/20 p-1.5 rounded" title="Copiar respuesta"><Copy size={14} /></button>
                          )}
                      </div>
                      ...
                      {msg.role === 'assistant' && msg.bookingData && msg.bookingData.isComplete && !bookingConfirmed && (
                          <div className="mt-3 bg-slate-800 border border-green-500/30 rounded-xl p-4 w-full animate-in slide-in-from-top-2 fade-in duration-500 shadow-xl">
                              ...
                              <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm mb-4">
                                  ...
                                  {msg.bookingData.stylePreference && (<div className="col-span-2 bg-slate-900/50 p-2 rounded-lg border border-slate-700/50"><span className="text-slate-500 text-xs block mb-0.5 flex items-center gap-1"><Scissors size={10}/> Estilo Solicitado</span><p className="text-amber-400 font-medium italic text-xs">"{msg.bookingData.stylePreference}"</p></div>)}
                              </div>
