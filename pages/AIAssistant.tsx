...
const AIAssistant: React.FC<AIAssistantProps> = ({ navigateToCalendar, isGuestMode = false }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(error);
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', text: 'Error de conexión. Intenta de nuevo.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateBooking = (data: ParsedBookingRequest) => {
    ...
    if (!check.isOpen) {
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', text: `⚠️ Error: ${check.reason}` }]);
        return;
    }
    ...
    addAppointment({
        ...
        date, time, price, status: initialStatus 
    });

    if (isGuestMode) {
        setBookingConfirmed(true);
        setLastBookingData(data);
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', text: '¡Solicitud enviada! Pendiente de confirmación.' }]);
    } else {
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', text: `¡Listo! Agendado para el ${date} a las ${time}.` }]);
        if(navigateToCalendar) setTimeout(() => navigateToCalendar(), 1500);
    }
  };
  ...
  const handleWhatsAppRedirect = () => {
    ...
    const message = `Hola! Reservé un turno:%0A📅 ${lastBookingData.date} a las ${lastBookingData.time}%0A👤 ${lastBookingData.clientName || 'Cliente'}`;
  ...
  if (isGuestMode && bookingConfirmed) {
      return (
          <div className="flex flex-col items-center justify-center h-full p-8 text-center animate-in zoom-in duration-500 relative">
              <button onClick={handleAdminLogin} className="absolute top-4 right-4 text-xs text-slate-600 hover:text-white flex items-center gap-1"><LogIn size={12}/> Soy Admin</button>
              <div className="bg-amber-500/10 p-8 rounded-full text-amber-500 mb-8 border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.2)]"><CalendarCheck size={80} /></div>
              ...
              {settings.contactPhone && (
                <button onClick={handleWhatsAppRedirect} className="w-full bg-green-600 hover:bg-green-500 text-white px-6 py-4 rounded-xl font-bold mb-4 flex items-center justify-center gap-3 shadow-lg shadow-green-900/20 active:scale-95 transition-all"><Smartphone size={20} /> Enviar Confirmación por WhatsApp</button>
              )}
              <button onClick={() => window.location.reload()} className="text-slate-500 font-medium hover:text-white transition-colors text-sm">Solicitar otro turno</button>
          </div>
      )
  }
      ...
      <div className={`flex-none mb-4 flex items-center justify-between ${isGuestMode ? 'border-b border-slate-800 pb-4' : ''}`}>
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                {isGuestMode ? <div className="bg-amber-500 p-2 rounded-lg text-slate-900"><Scissors size={20}/></div> : <Bot className="text-amber-500" />}
                {isGuestMode ? 'Reserva tu Turno' : 'BarberBot'}
            </h2>
            <p className="text-slate-400 text-sm mt-1">{isGuestMode ? 'Chatea con nuestra IA para ver disponibilidad.' : 'Tu asistente virtual (Modo Admin).'}</p>
          </div>
          {isGuestMode && (
              <button onClick={handleAdminLogin} className="text-xs bg-slate-800 text-slate-500 hover:text-amber-500 px-3 py-1.5 rounded-full border border-slate-700 transition-colors flex items-center gap-1" title="Acceso para el dueño"><LogIn size={14} /> <span className="hidden sm:inline">Soy Admin</span></button>
          )}
      </div>
          ...
          {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex flex-col max-w-[90%] md:max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                      <div className={`px-5 py-3.5 rounded-2xl relative shadow-md text-sm md:text-base ${msg.role === 'user' ? 'bg-amber-600 text-white rounded-tr-sm' : 'bg-slate-700 text-slate-100 rounded-tl-sm'}`}>
                          <p className="whitespace-pre-wrap leading-relaxed">{msg.text}</p>
                          {msg.role === 'assistant' && !isGuestMode && <button onClick={() => copyToClipboard(msg.text)} className="absolute top-2 right-2 opacity-0 hover:opacity-100 transition-opacity bg-black/10 hover:bg-black/20 p-1.5 rounded"><Copy size={14} /></button>}
                      </div>
                      <span className="text-[10px] text-slate-500 mt-1 px-1 opacity-70">{msg.role === 'user' ? 'Tú' : 'BarberBot'}</span>
                      {msg.role === 'assistant' && msg.bookingData && msg.bookingData.isComplete && !bookingConfirmed && (
                          <div className="mt-3 bg-slate-800 border border-green-500/30 rounded-xl p-4 w-full animate-in slide-in-from-top-2 fade-in duration-500 shadow-xl">
                              <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-700"><CheckCircle size={18} className="text-green-500"/><span className="text-slate-300 text-xs font-bold uppercase tracking-wider">{isGuestMode ? 'Confirmar Datos' : 'Turno Listo'}</span></div>
                              <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm mb-4">
                                  <div><span className="text-slate-500 text-xs block mb-0.5">Fecha</span><p className="text-white font-mono text-base">{msg.bookingData.date}</p></div>
                                  <div><span className="text-slate-500 text-xs block mb-0.5">Hora</span><p className="text-white font-mono text-base">{msg.bookingData.time}</p></div>
                                  {msg.bookingData.clientName && (<div className="col-span-2"><span className="text-slate-500 text-xs block mb-0.5">Nombre</span><p className="text-white font-medium">{msg.bookingData.clientName}</p></div>)}
                              </div>
                              <button onClick={() => handleCreateBooking(msg.bookingData!)} className="w-full bg-green-600 hover:bg-green-500 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-colors shadow-lg shadow-green-900/20 active:scale-95 transform duration-100"><CalendarPlus size={18} /> {isGuestMode ? '¡Enviar Solicitud!' : 'Agendar Turno'}</button>
                          </div>
                      )}
                  </div>
              </div>
          ))}
          {loading && <div className="flex justify-start"><div className="bg-slate-800 border border-slate-700 text-slate-400 px-4 py-3 rounded-xl rounded-tl-sm flex items-center gap-3 shadow-sm"><Loader2 className="animate-spin text-amber-500" size={18} /><span className="text-sm font-medium animate-pulse">{isGuestMode ? 'Consultando agenda...' : 'Analizando...'}</span></div></div>}
